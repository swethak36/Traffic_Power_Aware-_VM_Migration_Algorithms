...
public class BFHAlgorithm
{
	...
	public static double hostCreationEnergyBFH = 0.0;
	public static double migrationEnergyBFH = 0.0;
	static HashMap<Integer, Double> migrationPowerMappingBFH = new HashMap<Integer, Double>();
			...
			if (sumOfcpuUtilizations[hostIndex] > threshold[hostIndex])
			{
				...
				if (isAllocationSuccess && hosts[hostIndex].virtualMachine.length > 3)
				{
					...
					if (sumOfcpuUtilizations[hostIndex] > threshold[hostIndex])
					{
						...
						System.out.println("\n******************************  Available clusters in overloaded " + host.id + " ****************************");
	...
	private static void clusterHost(Host[] hosts, String hostID)
	{
		...
		// if CPU utilization is more than the threshold cluster the host
		System.out.println("");
		System.out.println("-----------------------------Clustering " + host.id + " since it is overloaded. -----------------------------------");
		for (int vmIndex = 0; vmIndex < host.virtualMachine.length; vmIndex++)
		{
			if (host.virtualMachine[vmIndex].connectedVMs != null)
			{
	...
	public static boolean allocateClusterToHost(Host hosts[], Host host, int clusterIndex)
	{
		...
		float migratingVMcpuUtilization = 0.0f;
		for (int index = 0; index < host.clusters[clusterIndex].length; index++)
		{
			migratingVMcpuUtilization += (float) (host.virtualMachine[host.clusters[clusterIndex][index]].cpuUtilization / 100);
		}
...
//		System.out.println("no.Of.VMs:::: " + noOfVms);
	...
	//	additionalHosts = additionalHostsBFH.size()/2; //swetha
		...
		migrationPowerMappingBFH.put(2, 4.0);
		migrationPowerMappingBFH.put(4, 4.7);
		migrationPowerMappingBFH.put(6, 5.4);
		migrationPowerMappingBFH.put(8, 6.0);
		...
		for (int index = 0; index < host.clusters[clusterIndex].length; index++)
		{
			...
			// System.out.println("<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			migrationEnergyBFH += migrationPowerMappingBFH.get((int) host.virtualMachine[host.clusters[clusterIndex][index]].ramUtilization);
	...
	{
		hostCreationEnergyBFH += 80; // swetha 80 --> 40 --> 20
